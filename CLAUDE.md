...
이 외에 **📋 보고서 탭**이 있다 — `reports/*.md`를 렌더하고 목록은 `reports/index.json`.

---

## 📋 보고서 에이전트 (무인 자동화)

발표일마다 미국 지표 해부 보고서를 자동 작성하는 시스템. 부품:

- **교본** `reports/CHECKLIST.md` — 지표별 전수 해부 골조. 보고서 품질 기준은 여기가 정본
- **상태** `reports/state.json` — 지표별 마지막 보고 데이터월. 미러 CSV의 최신 관측월과 비교해 신규 발표를 감지
- **데이터** `reports/data/*.csv` — GitHub Actions(fred-mirror, 평일 23:05 KST)가 FRED를 미러. 에이전트는 외부망 불필요
- **실행기** `.claude/run_report.ps1` — 작업 스케줄러 "EI 보고서 에이전트"(평일 23:40)가 호출, `claude -p` 헤드리스로 작성→커밋→푸시
- **로그** `.claude/report_agent.log`

**보고서가 안 올라오면 제일 먼저 로그를 봐라.** 2026-08-17~28에 열 번 연속 exit 255로
전패한 적이 있는데 원인은 **Claude CLI OAuth 만료**였다("Failed to authenticate" — 헤드리스는
로그인을 못 하므로 사용자가 터미널에서 `claude`를 열어 재로그인해야 풀린다).
스케줄러의 result=0은 ps1 껍데기 종료코드일 뿐이니 믿지 마라.

밀린 보고서를 세션에서 수동 보충할 때: 보고서 .md 작성 → `index.json` 맨 앞에 항목 추가 →
`state.json` 갱신 → 커밋. 파일명은 `YYYY-MM-DD_태그.md` (날짜 = **발표일**, 작성일 아님).
숫자는 반드시 `reports/data/*.csv`와 교차검증하고, 직전 같은 태그 보고서와 판정 연속성을 유지하라.
...
**KRX 조회는 반드시 `https://`.**
KRX는 `JSESSIONID`에 **Secure 플래그**를 붙인다. `http://`로 조회하면 세션 쿠키가 아예 실리지
않아 서버가 익명으로 보고 **전부 `400 LOGOUT`**을 뱉고, 게다가 그 평문 응답이 새 익명
JSESSIONID를 내려보내 **인증 쿠키까지 덮어쓴다**. 2026-09-17에 파생 4종이 통째로 멈춘 원인.
헷갈리는 이유는 **로그인은 CD001로 성공하고 페이지도 로그인 상태로 열린다**는 것 — 조회만
죽어서 세션 문제로 안 보인다. 진단할 땐 `s.cookies` 의 `secure` 플래그와, 요청에 실제로
`Cookie: JSESSIONID`가 실렸는지(`r.request.headers`)를 먼저 확인할 것.

**KRX 로그인이 `CD010 패스워드 변경 필요`를 내면 비번이 틀린 게 아니다.**
틀리면 `CD006 패스워드 불일치`가 온다. CD010은 "비번은 맞는데 변경해야 통과"라는 뜻이고,
**네이버 SNS 연동 로그인은 이 관문을 안 거치므로 멀쩡히 들어가진다**(그래서 더 헷갈린다).
KRX 화면의 "나중에" 버튼이 호출하는 `/contents/MDC/COMS/client/postponePasswordChange.cmd`
로 연기할 수 있지만 임시방편이다. 근본 해결은 비번 변경 + 레지스트리 `KRX_PW` 갱신.
